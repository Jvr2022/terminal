...
public:
    ...
    CConversionArea* GetConversionArea() const { return _pConversionArea; }
    ITfContext* GetInputContext() const { return _spITfInputContext.get(); }
    ...
    void SetFocus(bool fSet) const
    {
        if (!fSet && _cCompositions)
        {
            ...
            {
                if (const auto spCompositionServices = _spITfInputContext.try_query<ITfContextOwnerCompositionServices>())
                {
...
private:
    ...
    // Cicero stuff.
    TfClientId _tid = 0;
    wil::com_ptr<ITfThreadMgrEx> _spITfThreadMgr;
    wil::com_ptr<ITfDocumentMgr> _spITfDocumentMgr;
    wil::com_ptr<ITfContext> _spITfInputContext;
